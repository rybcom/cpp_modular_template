...
#pragma region operators 
		...
		Vector4 operator/(T rhs) const { return { x / rhs , y / rhs , z / rhs,w / rhs }; }
...
#pragma region operators 
		...
		Rotation operator/(double rhs) const { return { h / rhs , p / rhs , r / rhs }; }

		Rotation operator*(double rhs) const { return { h * rhs ,  p * rhs , r * rhs }; }

		Rotation operator+(double rhs) const { return { h + rhs ,  p + rhs , r + rhs }; }

		Rotation operator-(const Rotation& rhs) const { return { h - rhs.h ,p - rhs.p , r - rhs.r }; }

		Rotation operator+(const Rotation& rhs) const { return { h + rhs.h ,p + rhs.p , r + rhs.r }; }

		bool operator==(const Rotation& rhs) const
		{
			return cmp_floating(this->h, rhs.h) &&
				cmp_floating(this->p, rhs.p) &&
				cmp_floating(this->r, rhs.r);
		}
...
#pragma endregion
	...
	struct Transform
	{
		...
		Transform(Position const & pos, Rotation const & rot)
			:
			position{ pos },
			rotation{ rot }
		{}
...
#pragma region operators 
		...
		Transform operator/(double rhs) const { return { position / rhs , rotation / rhs }; }

		Transform operator*(double rhs) const { return  { position * rhs , rotation * rhs }; }

		Transform operator+(double rhs) const { return { position + rhs , rotation + rhs }; }

		Transform operator-(const Transform& rhs) const { return { position - rhs.position ,rotation - rhs.rotation }; }
